...
void DriveTrain::tankDrive(std::shared_ptr<Joystick> stick) {
	//tank drive by setting stick's raw axis values to motor controllers

	float left = stick.get()->GetRawAxis(1);//gets values, may need to negate left side instead if when going forward it goes backwards instead
...
void DriveTrain::driveForwardLeft(double speed) {
	frontLeft.get()->Set(-speed);
	backLeft.get()->Set(-speed);
}

void DriveTrain::driveBackwardLeft(double speed) {
	frontLeft.get()->Set(speed);
	backLeft.get()->Set(speed);
}

void DriveTrain::driveForwardRight(double speed) {
	frontRight.get()->Set(speed);
	backRight.get()->Set(speed);
}

void DriveTrain::driveBackwardRight(double speed) {
	frontRight.get()->Set(-speed);
	backRight.get()->Set(-speed);
}
